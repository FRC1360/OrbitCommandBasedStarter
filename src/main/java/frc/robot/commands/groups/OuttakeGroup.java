/*----------------------------------------------------------------------------*/
/* Copyright (c) 2018 FIRST. All Rights Reserved.                             */
/* Open Source Software - may be modified and shared by FRC teams. The code   */
/* must be accompanied by the FIRST BSD license file in the root directory of */
/* the project.                                                               */
/*----------------------------------------------------------------------------*/

package frc.robot.commands.groups;

import edu.wpi.first.wpilibj.command.CommandGroup;
import frc.robot.Robot;
import frc.robot.commands.claw.SetClawCommand;
import frc.robot.commands.elevator.ElevatorGoToTarget;
import frc.robot.commands.elevator.ElevatorGoToTarget;
import frc.robot.commands.elevator.ElevatorGoToTarget;
import frc.robot.commands.elevator.ElevatorGoToTarget;
import frc.robot.subsystems.Claw;

public class OuttakeGroup extends CommandGroup {
 
  public OuttakeGroup() {
   
    if(Robot.oi.getRightOperatorBumper())
    {
      addSequential(new SetClawCommand(Claw.Position.CLOSED_CLAW));
      Robot.elevator.setHoldPos(Robot.elevator.target - 100);
      Robot.elevator.moveToPosition();
    }
    if( Robot.oi.getLeftOperatorBumper())
    {
      addSequential(new OutakeCargo(true));
    }
  }
}
